namespace Tapas.Cms.FlatFile.Core.Infrastructure
{
    ...
    public class FlatFileCmsRazorProjectPageRouteModelProvider : IPageRouteModelProvider
    {
        private readonly FlatFileCmsGitOptions _flatFileOptions;
        private readonly ILogger<RazorProjectPageRouteModelProvider> _logger;
        private readonly RazorPagesOptions _pagesOptions;
        private readonly FlatFileCmsProviderRazorProjectFileSystem _razorFileSystem;
        private readonly IHostingEnvironment _hostingEnvironment;
        private readonly PageRouteModelFactory _routeModelFactory;
        private readonly string AreaRootDirectory;
        ...
        {
            foreach ( var item in _razorFileSystem.EnumerateItems( RootDirectory ) )
            {
                if ( !IsRouteable( item ) )
                {
                    continue;
                ...
                string relativePath = Path.GetFullPath( item.CombinedPath );
                ...
                if ( !PageDirectiveFeature.TryGetPageDirective( _logger, item, out string routeTemplate ) )
                {
        ...
        private void AddAreaPageModels( PageRouteModelProviderContext context )
        {
            foreach ( var item in _razorFileSystem.EnumerateItems( AreaRootDirectory ) )
            {
                ...
                string relativePath = item.CombinedPath;
                if ( context.RouteModels.Any( m => string.Equals( relativePath, m.RelativePath, StringComparison.OrdinalIgnoreCase ) ) )
                {
